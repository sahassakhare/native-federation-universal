...
import { SSRModuleLoader, SSRFederationUtils } from '../runtime/ssr-module-loader.js';
import { HydrationClient, AngularHydrationUtils } from '../runtime/hydration-client.js';
...
export class NativeFederationSSRService {
  ...
  private ssrLoader?: SSRModuleLoader;
  ...
  private async initializeSSR(): Promise<void> {
    this.ssrLoader = new SSRModuleLoader('./federation.manifest.json');
    await this.ssrLoader.initialize();
  }

  private async initializeClient(): Promise<void> {
    this.clientLoader = await AngularHydrationUtils.initializeForAngularSSR('./federation.manifest.json');
...
export class LoadFederatedComponentDirective implements OnInit, OnDestroy {
  ...
  private federationService = inject(NativeFederationSSRService);
...
export class AngularSSRHelpers {
   ...
   */
  static setupSSRLoader(manifestPath?: string): SSRModuleLoader {
    return new SSRModuleLoader(manifestPath, {
  ...
  static createUniversalRenderFunction(
    ...
    return async function renderWithFederation(options: any) {
      ...
      const ssrLoader = AngularSSRHelpers.setupSSRLoader(manifestPath);
 ...
 */
export function provideNativeFederationSSR(manifestPath?: string) {
  return [
    NativeFederationSSRService,
    {
